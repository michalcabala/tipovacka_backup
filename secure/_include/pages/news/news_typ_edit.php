...
$page_cz = $_POST['page_cz'] ?? ''; $page_en = $_POST['page_en'] ?? '';
...
<div class="card-body">
    <?php
    if($add == 0):
        $sql = "SELECT * FROM news_typ WHERE id = :id";
        $res = $pdo->prepare($sql);
        $res->execute(['id'=>$id]);
        $dev = $res->fetch();
        ...
        $valid = $dev["valid"]; $page_cz = $dev["page_cz"]; $page_en = $dev["page_en"];
    ...
    <?php
    elseif($add == 2):
        news_typ_edit($pdo, $id, $nazev_cz, $nazev_en, $poradi, $popis_cz, $popis_en, $page_cz, $page_en, $color, $valid);
    endif;
    ?>
</div>