...
$popis_cz = $_POST['popis_cz'] ?? "";   $popis_en = $_POST['popis_en'] ?? "";
$poradi = $_POST['poradi'] ?? 1;        $color = $_POST['color'] ?? ""; $page_cz = $_POST['page_cz'] ?? ''; $page_en = $_POST['page_en'] ?? '';
$add = $_POST['add'] ?? 0;
?>
<div class="card-body">
    <?php
        ...
        <form method="post">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-2">
                    ...
                    <button type="submit" class="form-control btn btn-primary">Vložit typ novinek</button>
                </div>
            </div>
        </form>
    <?php
    elseif($add == 1):
        news_typ_add($pdo, $nazev_cz, $nazev_en, $poradi, $popis_cz, $popis_en, $page_cz, $page_en, $color);
    endif;
    ?>
</div>