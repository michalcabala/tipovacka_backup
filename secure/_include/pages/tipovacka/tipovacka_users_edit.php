...
<div class="card-body">
    <?php
    if($add == 2): tipovacka_users_edit($pdo, $edit, $login, $name, $email, $password, $active, $blocked, $phpbb, $info_send, $valid); endif;
        $sql = "SELECT * FROM zdef_tipovacka_users WHERE id = :id";
        $res = $pdo->prepare($sql);
        $res->execute(['id'=>$edit]);
        $dev = $res->fetch();
    if ($add == 0):
        $login = $dev["login"]; $name = $dev["name"]; $email = $dev["email"]; $active = $dev["active"]; $blocked = $dev["blocked"]; $phpbb = $dev["phpbb"]; $info_send = $dev["info_send"]; $valid = $dev["valid"];
    endif;
    ?>
    <form method="post">
        <div class="form-row">
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-2">
                ...
                <button type="submit" class="form-control btn btn-primary">Uložit uživatele</button>
            </div>
